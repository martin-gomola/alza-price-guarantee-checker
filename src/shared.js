(function attachShared(root) {
  ...
  function getDefaultSearchShops(locale) {
    return shopCatalog?.getDefaultSearchShops(locale) || [];
  }
  ...
  function hasSearchTemplate(domain) {
    return Boolean(shopCatalog?.hasSearchTemplate(domain));
  }
  ...
  function buildSearchRequests(domain, query) {
    ...
    const templates = shopCatalog?.getSearchTemplates(normalizedDomain) || [];
    ...
    return queries.flatMap((requestQuery) => templates.map((template) => {
      if (typeof template === "string") {
        const url = applyQueryTemplate(template, normalizedDomain, requestQuery);
      ...
      const url = applyQueryTemplate(template.url, normalizedDomain, requestQuery);
      const displayUrl = applyQueryTemplate(template.displayUrl || template.url, normalizedDomain, requestQuery);
  ...
  function isManualOnlyShop(domain) {
    return Boolean(shopCatalog?.isManualOnly(domain));
  }
